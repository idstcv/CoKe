...
import coke.builder_double_view
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = coke.builder_double_view.CoKe(
    ...
    train_dataset = coke.folder.ImageFolder(
        traindir,
        coke.loader.DoubleCropsTransform(transforms.Compose(aug_1),
...
def train(train_loader, model, criterion, optimizer, epoch, args, scaler):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        with autocast():
            pred_view1, pred_view2, proj_view1, proj_view2, labels = model(images[0], images[1], target, epoch)
            loss_pred = 0
            loss_proj = 0
            with torch.no_grad():
                soft_label_view1 = []
            ...
            # compute loss on 2 224x224 views with 3 heads
            for j in range(0, args.coke_num_head):
            ...
            loss = (0.5 / (2. * args.coke_num_head)) * (loss_pred + loss_proj)
